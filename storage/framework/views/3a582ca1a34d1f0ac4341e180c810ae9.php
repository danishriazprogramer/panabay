    ...
    <?php $__env->startPush('scripts'); ?>
        ...
        >
            <div class="w-[455px] max-w-full max-sm:w-full">
                ...
                <div class="mt-5" v-for='(attribute, index) in childAttributes'>
                   <!-- START: NEW RADIO BUTTON TABS CODE -->
<template v-if="! attribute.swatch_type || attribute.swatch_type == '' || attribute.swatch_type == 'dropdown'">
    <!-- Attribute Label (e.g., "Size") -->
     <h2 class="mb-2 text-base font-medium text-zinc-800 lg:mb-4 lg:text-lg">
        {{ attribute.label }}:
    ...
    <!-- Container for the button-like options -->
    <div class="flex flex-wrap gap-x-4 gap-y-2">
        <!-- Loop through each option (e.g., 75 Inch, 88 Inch) -->
        <template v-for='(option, index) in attribute.options'>
            <!-- We only want to show actual options, not the "Select..." placeholder -->
            <div v-if="option.id">
                <input
                    type="radio"
                    ...
                    :disabled="attribute.disabled || option.disabled"
                    @change="configure(attribute, option.id)"
                />
                <label
                    :for="'attribute_' + attribute.id + '_' + option.id"
                    class="block cursor-pointer select-none rounded-md border border-zinc-300 px-5 py-2 text-center text-sm font-medium transition-all peer-checked:border-amber-400 peer-checked:bg-navyBlue peer-checked:text-white peer-disabled:cursor-not-allowed peer-disabled:bg-zinc-100 peer-disabled:opacity-50"
                    v-text="option.label"
                ></label>
            </div>
        </template>
                                    ...
                                    >
                                        ...
                                        >
                                            <input
                                                ...
                                                :checked="option.id == attribute.selectedValue"
    ...
    getSelectedOptionLabel(attribute) {
        // Return nothing if no option is selected yet
        if (!attribute.selectedValue) {
            return '';
        }

        // Find the full option object using the selected ID
        const selectedOption = attribute.options.find(option => option.id == attribute.selectedValue);

        // Return the label if the option was found, otherwise return an empty string
...
<?php endif; ?><?php /**PATH /var/www/html/resources/themes/default/views/products/view/types/configurable.blade.php ENDPATH**/ ?>